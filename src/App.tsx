...
import { ArrowDownWideNarrow, ArrowLeft, ArrowUpRight, BellOff, BookOpen, Bookmark, Check, CheckCheck, ChevronRight, CircleHelp, Clock3, ExternalLink, Folder, HardDrive, Inbox, LayoutGrid, LoaderCircle, LockKeyhole, Paperclip, Plus, RefreshCw, Search, Settings2, ShieldCheck, Sparkles, WifiOff, X } from 'lucide-react';
...
export default function App() {
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <div className="workspace-label">나만의 작은 인터넷 서재</div>
      ...
      <div className="sidebar-bottom"><div className="local-note"><span className="local-note-icon"><HardDrive size={19} /></span><strong>좋은 글은, 내 곁에.</strong><p>마음에 드는 글을 모아<br />나만의 보관함을 채워보세요.</p><span className="local-note-foot"><ShieldCheck size={12} /> 데모 상태는 이 기기에 보관</span></div>
        ...
        <div className="profile"><span className="avatar">나</span><div><strong>개인 작업 공간</strong><small>LOCAL WORKSPACE</small></div><span className="online-dot" /></div>
      </div>
    </aside>

    <main className="main">
      <header className="topbar"><div className="breadcrumb">내 작업 공간<ChevronRight size={13} /><span>{title}</span></div><div className="topbar-right"><span className="demo-badge"><span />데모 모드</span><button className="icon-btn" aria-label="데모 모드 안내" onClick={() => setHelpOpen(true)}><CircleHelp size={18} /></button></div></header>
      <div className="page-heading"><div><div className="eyebrow">YOUR PERSONAL READING SPACE</div><h1>{title}<span className="heading-dot">.</span></h1><p>{view === 'saved' ? '다시 읽고 싶은 이야기, 이곳에 모아두세요.' : view === 'settings' ? '나에게 편한 읽기 환경을 만들어보세요.' : '여러 곳의 이야기, 이제 한곳에서 만나보세요.'}</p></div><button className="primary-button" onClick={() => setAddOpen(true)}><Plus size={17} />게시판 추가</button></div>
      ...
      {view === 'settings' ? <Settings state={state} setState={setState} /> : <>
        <div className="collection-summary"><span><span className="green-dot" />{knownBoards.filter(b => b.source !== 'unsupported').length}개의 데모 게시판</span><span className="summary-divider" /><span><Clock3 size={13} />{lastRefresh ? `${new Date(lastRefresh).toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit' })} 샘플 확인` : '샘플 데이터가 준비되어 있어요'}</span><button onClick={refresh} disabled={loading} className="refresh-button"><RefreshCw size={14} className={refreshing ? 'spinning' : ''} />새로고침</button></div>
        <div className="content-panel">
          <div className="panel-toolbar"><div className="view-tabs"><span className="active-tab">{view === 'saved' ? '보관한 글' : '글 목록'}<b>{visible.length}</b></span><span className="sample-label">샘플</span></div><div className="toolbar-actions"><label className="search-box"><Search size={16} /><input aria-label="글 검색" placeholder={view === 'saved' ? '보관한 글 검색' : '관심 있는 글을 검색해 보세요'} value={query} onChange={e => { setQuery(e.target.value); setSelected(null); }} />{query && <button className="icon-btn" aria-label="검색어 지우기" onClick={() => setQuery('')}><X size={14} /></button>}</label><button className="icon-btn mark-all" title="모두 읽음으로 표시" aria-label="모두 읽음으로 표시" onClick={markVisible} disabled={!visible.length}><CheckCheck size={19} /></button></div></div>
      ...
      <footer className="app-footer"><span><span className="green-dot" />로컬 작업 공간<span className="footer-divider">/</span>서버 전송 없음</span><label>화면 상태 미리보기<select aria-label="화면 상태 미리보기" value={scenario} onChange={e => { setScenario(e.target.value as Scenario); if (view === 'settings') setView('all'); if (e.target.value === 'partial') { setQuery(''); setBoardFilter('all'); setView('all'); setSelected('demo-1'); setDetailMobile(true); } }}><option value="normal">기본 화면</option><option value="empty">빈 목록</option><option value="loading">로딩</option><option value="auth">인증 만료</option><option value="partial">부분 저장</option></select></label><span className="version">LocalClip v0.1</span></footer>
    </main>
    ...
    {helpOpen && <Modal title="좋은 글을 모으는 작은 공간" close={() => setHelpOpen(false)}><div className="help-content"><span className="brand-icon"><Paperclip size={28} /></span><h3>반가워요, LocalClip입니다.</h3><p>여러 게시판의 이야기를 한곳에서 읽고, 마음에 드는 글을 내 PC에 간직하기 위한 앱입니다.</p><div className="info-box"><strong>지금은 UI 데모예요</strong><p>검색, 읽음 표시, 게시판 등록과 샘플 보관함을 체험할 수 있습니다. 설정과 보관 상태는 이 기기에 저장됩니다.</p><p>실제 로그인·글 수집·본문 및 이미지 다운로드·저장 폴더 선택은 아직 연결되지 않았습니다. 가상 글의 원문 열기는 실제 출처 게시판을 엽니다.</p></div><button className="primary-button full-width" onClick={() => setHelpOpen(false)}>둘러보기<ArrowUpRight size={16} /></button></div></Modal>}
...
function AddBoard({ close, existing, add }: { close(): void; existing: Board[]; add(b: Omit<Board, 'id'>): void }) {
  ...
  return <Modal title="새 게시판 추가" close={close}><form onSubmit={submit}><p className="modal-description">즐겨 찾는 게시판을 나만의 목록에 추가하세요.</p><label className="form-field">게시판 이름<input autoFocus maxLength={60} placeholder="예: 디아블로2 자유 게시판" value={name} onChange={e => { setName(e.target.value); setPreview(null); }} required /></label><label className="form-field">게시판 주소<input type="url" placeholder="https://" value={url} onChange={e => { setURL(e.target.value); setPreview(null); }} required /></label><div className="example-links"><span>예시 입력</span><button type="button" onClick={() => { setName('디아2 자유 게시판'); setURL('https://www.inven.co.kr/board/diablo2/5735'); setPreview(null); }}>인벤</button><button type="button" onClick={() => { setName('AAGAG 최신 이슈'); setURL('https://aagag.com/'); setPreview(null); }}>AAGAG</button></div>{error && <p className="form-error" role="alert">{error}</p>}{preview && <div className="board-preview"><SourceIcon board={{ ...preview, id: 'preview' }} /><div><strong>{preview.name}</strong><p>{preview.source === 'unsupported' ? '이 사이트는 지원 준비 중입니다.' : `${sourceName({ ...preview, id: '' })} 주소를 확인했어요. 실제 목록 수집은 연결 예정입니다.`}</p></div></div>}<div className="info-box compact"><ShieldCheck size={17} /><span>지금은 주소와 이름만 로컬에 저장합니다.<br />로그인 정보는 입력하지 마세요.</span></div><div className="modal-footer"><button type="button" className="secondary-button" onClick={close}>취소</button><button className="primary-button" type="submit">{preview ? '게시판 추가' : '미리보기'}<ChevronRight size={15} /></button></div></form></Modal>;
}
function Settings({ state, setState }: { state: DemoState; setState: React.Dispatch<React.SetStateAction<DemoState>> }) {
  return <section className="settings-panel" aria-label="앱 설정"><div className="settings-intro"><Settings2 size={21} /><div><h2>내 작업 공간 설정</h2><p>데모 설정은 이 기기에 자동으로 저장됩니다.</p></div><span className="demo-badge">데모</span></div><div className="setting-row"><div><h3>목록 갱신 간격</h3><p>실행 중 샘플 갱신을 시뮬레이션합니다. 실제 수집은 연결 예정입니다.</p></div><select aria-label="갱신 간격" value={state.preferences.interval} onChange={e => setState(s => ({ ...s, preferences: { ...s.preferences, interval: Number(e.target.value) } }))}>{[1, 5, 10, 30].map(n => <option key={n} value={n}>{n}분마다</option>)}</select></div><div className="setting-row"><div><h3>로컬 저장 위치</h3><p>{state.preferences.storageLabel}</p><p>현재 보관함은 앱의 데모 상태 저장소를 사용합니다.</p></div><button className="secondary-button" disabled><Folder size={15} />폴더 선택 · 연결 예정</button></div><div className="setting-row"><div><h3>사이트 연결</h3><p>실제 로그인과 세션 보관 기능은 다음 단계에 연결됩니다.</p></div></div><div className="connection-list">{['인벤', 'AAGAG'].map((name, i) => <div key={name}><SourceIcon board={{ id: '', name, url: '', source: i ? 'aagag' : 'inven' }} /><strong>{name}</strong><span>미연결</span><button className="secondary-button" disabled><LockKeyhole size={14} />로그인 · 연결 예정</button></div>)}</div><div className="setting-row"><div><h3>알림</h3><p>이번 버전에서는 알림을 보내지 않습니다.</p></div><span className="muted-chip"><BellOff size={14} />연결 예정</span></div><div className="settings-note"><ShieldCheck size={18} /><p>데모 상태는 이 기기에만 저장합니다. 계정, 비밀번호, 실제 게시글을 수집하거나 서버에 전송하지 않습니다.</p></div></section>;
}
